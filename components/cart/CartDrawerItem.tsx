...
export function CartDrawerItem({ item, itemPrice, itemImage, onRemove, onDecrement, onIncrement }: CartDrawerItemProps) {
    return (
        <div className="flex w-full min-w-0 gap-3 rounded-2xl p-3! transition-colors duration-150 hover:bg-text-primary/10!">
            <div className="flex h-16 w-16 shrink-0 items-center justify-center rounded-xl border border-black/5 bg-transparent shadow-sm">
            ...
            <div className="flex min-w-0 flex-1 flex-col justify-between py-0.5!">
                <div className="flex min-w-0 items-start justify-between gap-1">
                    <div className="min-w-0 flex-1">
                        <p className="truncate text-sm font-semibold leading-tight text-text-primary">{item.product.name}</p>
                        <div className="mt-1! flex min-w-0 items-center gap-1.5">
                            ...
                            <span className="truncate text-xs text-text-primary/90">
                    ...
                    <Button
                        ...
                        size="icon"
                        onClick={onRemove}
                        className="shrink-0 h-7 w-7 bg-transparent! p-1! text-text-primary transition-colors hover:bg-transparent! hover:text-red-500"
                    >
                        <Trash2 size={14} />
                    </Button>
                </div>
                <div className="mt-2! flex items-center justify-between gap-2">
                    <span className="truncate text-sm font-display font-bold text-text-primary">{formatVND(itemPrice * item.quantity)}</span>
                    <div className="flex shrink-0 items-center gap-1 rounded-xl border border-white/10 bg-white/8 px-1! py-0.5!">
